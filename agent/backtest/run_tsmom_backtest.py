...
训练集：BTC/SOL/DOGE/PEPE/WIF/BONK/ARB/OP/TIA (9币种)
样本外：ETH/AVAX/LINK/NEAR/APT/FIL/INJ/ATOM/ADA/XRP/LTC/TRX (12币种)
参数：lookback=120(20天), 不做参数优化，先验值
"""
...
def run_backtest(symbols, data_4h, label):
    ...
    signal_engine = TSMOMSignalEngine(
        lookback=120,
        hurst_filter=True,
        hurst_window=200,
    ...
    if len(bt.trades) >= 10:
        ...
        print(f"\n    样本不足 ({len(bt.trades)} < 10)")
        return metrics, bt.trades, None, None, {"p_value": None, "significant": False}


def main():
    print("=" * 80)
    print("TSMOM 策略回测 (4H, Hurst filter, 结构位止损)")
    print("=" * 80)

    train_symbols = ["BTC-USDT", "SOL-USDT", "DOGE-USDT", "PEPE-USDT", "WIF-USDT",
                     "BONK-USDT", "ARB-USDT", "OP-USDT", "TIA-USDT"]
    test_symbols = ["ETH-USDT", "AVAX-USDT", "LINK-USDT", "NEAR-USDT", "APT-USDT",
                    "FIL-USDT", "INJ-USDT", "ATOM-USDT", "ADA-USDT", "XRP-USDT", "LTC-USDT", "TRX-USDT"]

    print(f"\n加载缓存数据 (360天 4H)...")
    data_4h = {}
    for s in train_symbols + test_symbols:
        inst_id = f"{s}-SWAP"
        df = fetch_klines(inst_id, bar="4H", days=360)
        if not df.empty:
            data_4h[s] = df
    print(f"  {len(data_4h)} 个币种")

    # === 训练集 ===
    print(f"\n{'='*80}")
    print(f"训练集 ({len([s for s in train_symbols if s in data_4h])} 币种)")
    print(f"{'='*80}")
    train_data = {s: data_4h[s] for s in train_symbols if s in data_4h}
    train_metrics, train_trades, train_fk, train_ci, train_wilcox = run_backtest(
        [s for s in train_symbols if s in data_4h], train_data, "训练集"
    )

    # === 样本外 ===
    print(f"\n{'='*80}")
    print(f"样本外 ({len([s for s in test_symbols if s in data_4h])} 币种)")
    print(f"{'='*80}")
    test_data = {s: data_4h[s] for s in test_symbols if s in data_4h}
    test_metrics, test_trades, test_fk, test_ci, test_wilcox = run_backtest(
        [s for s in test_symbols if s in data_4h], test_data, "样本外"
    )
    ...
    print(f"  {'-'*44}")
    print(f"  {'交易次数':<20} {len(train_trades):>12} {len(test_trades):>12}")
    print(f"  {'最终权益':<20} {train_metrics['final_equity']:>12.1f} {test_metrics['final_equity']:>12.1f}")
    print(f"  {'总收益%':<20} {train_metrics['total_return_pct']:>+12.1f} {test_metrics['total_return_pct']:>+12.1f}")
    print(f"  {'胜率%':<20} {train_metrics['win_rate_pct']:>12.1f} {test_metrics['win_rate_pct']:>12.1f}")
    print(f"  {'盈利因子':<20} {train_metrics['profit_factor']:>12.2f} {test_metrics['profit_factor']:>12.2f}")
    print(f"  {'最大回撤%':<20} {train_metrics['max_drawdown_pct']:>12.1f} {test_metrics['max_drawdown_pct']:>12.1f}")
    print(f"  {'平均持仓bars':<20} {train_metrics['avg_holding_bars']:>12.1f} {test_metrics['avg_holding_bars']:>12.1f}")
    if train_fk is not None and test_fk is not None:
        print(f"  {'f_kelly':<20} {train_fk:>+12.4f} {test_fk:>+12.4f}")
    if train_ci is not None and test_ci is not None:
        print(f"  {'Bootstrap CI下界%':<20} {train_ci*100:>+12.2f} {test_ci*100:>+12.2f}")
    print(f"  {'Wilcoxon p':<20} {str(train_wilcox['p_value']):>12} {str(test_wilcox['p_value']):>12}")

    # 最终判定
    print(f"\n  {'='*40}")
    if test_fk is not None and test_fk > 0:
        print(f"  [PASS] 样本外 f_kelly > 0, edge 可能真实存在")
    elif test_fk is not None and test_fk > -0.05:
        print(f"  [BORDERLINE] 样本外 f_kelly 接近 0, 需要更多数据")
    else:
        print(f"  [FAIL] 样本外 f_kelly < 0")
